...
class PairwiseImageDatasetProcessor(DatasetProcessor):
    def _encode_data_example(
        self,
        prompt: list[dict[str, str]],
        response: list[dict[str, str]],
        ...
        chosen_messages = self.template.mm_plugin.process_messages(
            prompt + [response[0]], [images[0], images[1]], videos, audios, self.processor
        )
        rejected_messages = self.template.mm_plugin.process_messages(
            prompt + [response[1]], [images[0], images[2]], videos, audios, self.processor
        )
        prompt_ids, chosen_ids = self.template.encode_oneturn(self.tokenizer, chosen_messages, system, tools)
        _, rejected_ids = self.template.encode_oneturn(self.tokenizer, rejected_messages, system, tools)
        ...
        chosen_input_ids = prompt_ids + chosen_ids
        chosen_labels = [IGNORE_INDEX] * (source_len + target_len)
        rejected_input_ids = prompt_ids + rejected_ids
        rejected_labels = [IGNORE_INDEX] * (source_len + target_len)
        return chosen_input_ids, chosen_labels, rejected_input_ids, rejected_labels
    ...
    def preprocess_dataset(self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        ...
        for i in range(len(examples["_prompt"])):
            ...
            model_inputs["rejected_attention_mask"].append([1] * len(rejected_input_ids))
            model_inputs["rejected_labels"].append(rejected_labels)
            model_inputs["chosen_images"].append([examples["_images"][i][0], examples["_images"][i][1]])
            model_inputs["chosen_videos"].append([])
            model_inputs["chosen_audios"].append([])
            model_inputs["rejected_images"].append([examples["_images"][i][0], examples["_images"][i][2]])
            model_inputs["rejected_videos"].append([])
            model_inputs["rejected_audios"].append([])
