...
class PairwiseTrainer(Trainer):
    ...
    def compute_loss(
        ...
        chosen_scores_normalized = torch.sigmoid(chosen_scores)
        rejected_scores_normalized = torch.sigmoid(rejected_scores)

        loss = -torch.nn.functional.logsigmoid(chosen_scores.float() - rejected_scores.float()).mean()
        # loss = -torch.nn.functional.logsigmoid(chosen_scores_normalized.float() - rejected_scores_normalized.float()).mean()
        if return_outputs:
            return loss, (loss, chosen_scores_normalized, rejected_scores_normalized)
        else:
            return loss
