...
class ComputeAccuracy:
    ...
    def _dump(self) -> Optional[dict[str, float]]:
        ...
        self.score_dict = {"accuracy": [], "chosen_scores": [], "rejected_scores": []}
        return result
    ...
    def __call__(self, eval_preds: "EvalPrediction", compute_result: bool = True) -> Optional[dict[str, float]]:
        ...
        if not chosen_scores.shape:
            ...
            self.score_dict["chosen_scores"].append(chosen_scores)
            self.score_dict["rejected_scores"].append(rejected_scores)
        else:
            for i in range(len(chosen_scores)):
                self.score_dict["accuracy"].append(chosen_scores[i] > rejected_scores[i])
                self.score_dict["chosen_scores"].append(chosen_scores[i])
                self.score_dict["rejected_scores"].append(rejected_scores[i])
