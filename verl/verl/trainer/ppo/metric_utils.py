...
def compute_reward_metrics(batch: DataProto):
    if 'reward_parts' not in batch.non_tensor_batch.keys():
        return {}

    reward_parts = batch.non_tensor_batch.pop('reward_parts')
    var2vals = {}
    for i in range(len(batch)):
        for key, value in reward_parts[i].items():
            if isinstance(value, (str, list)):
                continue
            if value == -100:
                continue
            if key not in var2vals:
                var2vals[key] = []
            var2vals[key].append(value)

    # 计算每个部分的平均值，方差，最大值和最小值
    reward_metrics = {f"reward/{key}/mean": np.mean(values) for key, values in var2vals.items()}
    reward_metrics.update({f"reward/{key}/std": np.std(values) for key, values in var2vals.items()})
    reward_metrics.update({f"reward/{key}/min": np.min(values) for key, values in var2vals.items()})
    reward_metrics.update({f"reward/{key}/max": np.max(values) for key, values in var2vals.items()})
...
def process_validation_metrics(data_sources: list[str], sample_inputs: list[str], infos_dict: dict[str, list[Any]], seed: int = 42) -> dict[str, dict[str, dict[str, float]]]:
    ...
    for sample_idx, data_source in enumerate(data_sources):
        ...
        for var_name, var_vals in infos_dict.items():
            if isinstance(var_vals[sample_idx], dict):
                for sub_var_name, sub_var_val in var_vals[sample_idx].items():
                    var2vals[f"{var_name}/{sub_var_name}"].append(sub_var_val)
            else:
    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if isinstance(var_vals[0], (str, list)):
                    continue
