...
def compute_grpo_extra_advantage(
    token_level_rewards: torch.Tensor,
    ...
    """
    Compute advantage for GRPO, operating on Outcome reward and an extra advantage tensor.
    Args:
        token_level_rewards: `(torch.Tensor)`
            shape is (bs, response_length)
        extra_advantage_tensor: `(torch.Tensor)`
    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                raise ValueError(f"no score in prompt index: {idx}")
        std_tensor = torch.tensor([id2std[index[i]] for i in range(bsz)])
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                scores[i] = (scores[i] - id2mean[index[i]]) / (id2std[index[i]] + epsilon)
            else:
                scores[i] = scores[i] - id2mean[index[i]]
        scores = scores.unsqueeze(-1) * response_mask   # response_mask 使得 scores 的 shape 为(bs, response_length)
        scores += (extra_advantage_tensor / (std_tensor + epsilon).unsqueeze(-1)) * response_mask  # 将额外的优势张量添加到 scores 中

    return scores, scores


def compute_grpo_first_extra_advantage(
    token_level_rewards: torch.Tensor,
    extra_advantage_tensor: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
