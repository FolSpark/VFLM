...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        from bitsandbytes.optim import Adam8bit, AdamW8bit, SGD8bit, Lion, Lion8bit
        ...
        self.processor = hf_processor(local_path, trust_remote_code=trust_remote_code, **self.config.get("processor_kwargs", {}))
        ...
        if role == "actor" and optim_config is not None:
            ...
            if optim_name =='AdamW8bit':
                optim_fun = AdamW8bit
            elif optim_name =='Adam8bit':
                optim_fun = Adam8bit
            else:
                optim_fun = optim.AdamW
            actor_optimizer = optim_fun(
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        self.processor = hf_processor(tokenizer_path, trust_remote_code=config.model.get("trust_remote_code", False), **self.config.get("processor_kwargs", {}))
        ...
        optim_name = config.optim.get('name', 'AdamW')
        if optim_name =='AdamW8bit':
            optim_fun = AdamW8bit
        elif optim_name =='Adam8bit':
            optim_fun = Adam8bit
