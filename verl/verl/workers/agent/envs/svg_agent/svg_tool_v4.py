...
class SvgToolEnv(ToolBase):
    ...
    def extract_action(self, action_string: str) -> Dict[str, Any]:
        ...
        return tool_call_match.group(1).strip() if tool_call_match else None 
    ...
    def extract_tool_and_svg(self, text: str):
        ...
        if not (params_string.startswith("<<<") and params_string.endswith(">>>")):
            return tool_name, None
        else:
            svg_code = params_string[3:-3].strip()
            return tool_name, svg_code
        ...
        try:
            ...
            if answer:
                ...
                if step == 0:
                    ...
                    reward = image_score - max(tool_infos["tool_result_images_scores"]) if step > 0 else 0.0
            ...
            if tool_name != "svg_to_image_tool" or svg_code is None:
            ...
            if is_last_step:
                reward -= 0.2  # Slightly penalize the last step to encourage earlier tool use
                return "", reward, True, {"image_score": image_score, "origin_multi_modal_data": {"image": [rendered_img]}}
