...
from verl.utils.svg_utils import export_svg_to_img, export_svg_with_bg, extract_svg_text
...
class SvgToolEnv(ToolBase):
    ...
    def __init__(self, _name, _desc, _params, **kwargs):
        ...
        # self.servers = "http://120.92.164.165:30079/compute_score"
        self.servers = "http://10.244.224.221:8080/compute_score"
    ...
    def compute_image_score(self, svg_code, text_content, bg_img_b64_str, rendered_img_b64_str) -> dict[str: float]:
        for cnt in range(3):  # Retry up to 3 times
            try:
                ...
                return {"image_score": image_score, "ocr_accuracy": ocr_accuracy, "svg_code_accuracy": svg_code_accuracy}
    ...
    def execute(self, action_string: str, tool_infos, step: int, is_last_step: bool, **kwargs) -> tuple:
        ...
        try:
            ...
            if answer:
                format_reward = 0.1 if is_valid else -0.1
                ...
                try:
                    ...
                    rendered_img = Image.new('RGB', (224, 224), color='black')
                score_dict = self.compute_image_score(
                    answer,
                    self.text_content,
                    self.get_image_b64_str(self.multi_modal_data['image'][0]),
                    self.get_image_b64_str(rendered_img)
                )
                image_score = score_dict["image_score"]
                ocr_accuracy = score_dict["ocr_accuracy"]
                svg_code_accuracy = score_dict["svg_code_accuracy"]
                
                total_score = image_score + 0.25 * ocr_accuracy + 0.25 * svg_code_accuracy
                ...
                if step == 0:
                    ...
                    if step > 0:
                        ...
                        prev_max = max(tool_infos["tool_result_images_scores"], key=lambda x: x.get("total_score", 0))["total_score"]
                        if total_score >= prev_max:
                            reward = 0.7
                        else:
                            reward = total_score - prev_max
                    else:
                        reward = 0.0
                reward += format_reward
                # return "", reward, True, {"image_score": image_score, "origin_multi_modal_data": {"image": [rendered_img]}}
                # return "", reward, True, {"total_score": total_score, "image_score": image_score, "ocr_accuracy": ocr_accuracy, "svg_code_accuracy": svg_code_accuracy, "origin_multi_modal_data": {"image": [rendered_img]}}
                score_dict["format_reward"] = format_reward
                return "", format_reward, True, {"image_score": score_dict, "origin_multi_modal_data": {"image": [rendered_img]}}
            ...
            if step == 0:
                ...
                prev_max = max(tool_infos["tool_result_images_scores"], key=lambda x: x.get("total_score", 0))["total_score"]
                reward = total_score - prev_max
                # if total_score >= prev_max - 0.05:
                #     reward = total_score - prev_max
                # else:
                #     reward = total_score - prev_max - 0.05  # Slightly penalize if the score is significantly lower than the previous max
            reward += format_reward
            score_dict["format_reward"] = format_reward
            if is_last_step:
            ...
            # info = {"status": "success", "tool_used": tool_name, "total_score": total_score, "image_score": image_score, "ocr_accuracy": ocr_accuracy, "svg_code_accuracy": svg_code_accuracy, "origin_multi_modal_data": {"image": [rendered_img]}}
            info = {"status": "success", "tool_used": tool_name, "image_score": score_dict, "origin_multi_modal_data": {"image": [rendered_img]}}
            # print(f'[DEBUG] SUCCESS ACTION {action_string=}')
            return obs, format_reward, done, info
            ...
            # info = {"error": str(e), "status": "failed", "total_score": total_score, "image_score": image_score, "ocr_accuracy": ocr_accuracy, "svg_code_accuracy": svg_code_accuracy, "origin_multi_modal_data": {"image": [rendered_img]}}
            info = {"error": str(e), "status": "failed", "image_score": score_dict, "origin_multi_modal_data": {"image": [rendered_img]}}
            return obs, format_reward, done, info
