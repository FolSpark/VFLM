...
from verl.utils.constants import IMAGE_PLACEHOLDER, VIDEO_PLACEHOLDER
...
def _merge_multi_modal_inputs(mm_input, other):
    ...
    for key in mm_input.keys():
        ...
        if isinstance(mm_value, np.ndarray) and isinstance(other_value, np.ndarray):
            ...
            if key == "image_grid_thw": # FIXME: Why are some of them float64?
                merged_value = torch.cat([mm_value, other_value], dim=0).type(torch.LongTensor)
            else:
...
def _preprocess_multi_modal_inputs(prompt_str, processor, **kwargs):
    ...
    input_mm_data["image"] = [process_image(img, max_pixels=1024 * 28 * 28) for img in input_mm_data["image"]]
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    if custom_stop:
        ...
        print(f' [DEBUG stop] {prev_stop=}, {custom_stop=}, {agent_sampling_params.stop=}')
    ...
    processor = hf_processor(config.agent.vl_model_path, **config.agent.get("processor_kwargs", {}))
    ...
    for step in range(config.agent.max_turns):
        ...
        active_tool_infos = [tool for tool, is_active in zip(tool_result_list, active_mask) if is_active]
        ...
        if pg.is_first_rank:
            obs_results = env.step(active_indices, actions, active_tool_infos, step, is_last_step=(step == config.agent.max_turns - 1))
        else:
            obs_results = None
        ...
        for idx, obs, act, rew, done, info in zip(active_indices, observations, actions, rewards, dones, infos):
            ...
            if "tool_result_images_scores" not in tool_result_list[idx]:
                tool_result_list[idx]["tool_result_images_scores"] = []
            tool_result_list[idx]["tool_result_images_scores"].append(info["image_score"])
            # if 'origin_multi_modal_data' not in tool_result_list[idx]:
            #     tool_result_list[idx]["origin_multi_modal_data"] = {"image": []}
            # tool_result_list[idx]["origin_multi_modal_data"]["image"] += info["origin_multi_modal_data"]["image"]
...
def execute_tool_call(sample, tokenizer=None, processor=None, pbar=None, step=0, is_last_step=False):
    action_string = sample.get('action', '')
    tool = sample.get('tool', None)
    tool_infos = sample.get('tool_infos', {})

    # non-agent data
    if action_string == '' or tool is None:
        print(f'[DEBUG] No action_string or tool found in sample')
        return {}, 0.0, True, {}

    tool_result, reward, done, info = tool.execute(action_string, tool_infos, step, is_last_step=is_last_step)
...
class ParallelEnv:
    ...
    def step(self, active_indices, actions, active_tool_infos, step, is_last_step=False):
        """
        Input:
        ...
        for i, (idx, act) in enumerate(zip(active_indices, actions)):
            ...
            if act.outputs[0].finish_reason == 'length':
                print(f'[DEBUG] Action {i} finished by length')
                is_done = True
                # continue

            if len(act.outputs[0].token_ids) == 0:
                print(f'[DEBUG] Action {i} has no token_ids')
                is_done = True
                # continue
        ...
        for i, idx, action in zip(real_indices, valid_indices, valid_actions):
            agent_inputs.append(dict(
                ...
                action=action,
                tool=self.tools[idx],
                tool_infos=active_tool_infos[i],
            ))
        ...
        if num_workers <= 1:
            ...
            for agi in agent_inputs:
                subidx = agi['idx']
                obs, reward, done, info = execute_tool_call(agi, self.tokenizer, self.processor, pbar=pbar, step=step, is_last_step=is_last_step)
                obs_list[subidx] = obs
                reward_list[subidx] = reward
                done_list[subidx] |= done
                info_list[subidx] = info
        else:
            partial_tool_func = partial(execute_tool_call, tokenizer=self.tokenizer, processor=self.processor, pbar=None, step=step, is_last_step=is_last_step)
            with multiprocessing.Pool(processes=num_workers) as pool:
                raw_outputs = list(tqdm(pool.imap(partial_tool_func, agent_inputs), total=len(valid_actions), desc=f'Tool calling on {num_workers} workers'))
            for agi, raw in zip(agent_inputs, raw_outputs):
                ...
                reward_list[subidx] = reward
                done_list[subidx] |= done
                # if 'image_score' in info:   # multiprocessing 进程变量不共享
                #     agi["tool"].tool_result_images_scores.append(info['image_score'])
                # if 'origin_multi_modal_data' in info and 'image' in info['origin_multi_modal_data']:
                #     agi["tool"].tool_result_images.append(info["origin_multi_modal_data"]["image"][0])
                info_list[subidx] = info
            ...
            for _ in range(n):
                if tool_name:
                    ...
                    reset_output = tool_fns.reset(
                        ...
                        **data_item.non_tensor_batch["tools_kwargs"],
                    )
