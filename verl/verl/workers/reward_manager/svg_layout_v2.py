...
class SvgLayoutRewardManager:
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        for i in range(len(data)):
            ...
            indexed_data[extra_info["index"]].append(1 if len(tool_result_images) > 1 else 0)
        ...
        # RaRTK = sum(1 for p in  params if p["tool_result_images"] > 1) / len(params)
        RaRTK = {}
        ...
        with multiprocessing.Pool(processes=min(self.api_num, len(data))) as pool:
            for score in tqdm(pool.imap(self.compute_score, params), total=len(params), desc="Computing reward scores"):
                ...
                reward_tensor[score["idx"], valid_response_length - 1] = reward
    ...
    def compute_score(self, param) -> Dict[str, Any]:
        ...
        Returns:
            A dictionary containing the idx and the computed score.
        """
        ...
        RaRTK_reward = max(0.0, self.RaRTK_threshold - RaRTK) if len(tool_result_images) > 1 else 0.0

        return {
            "idx": param["idx"],
            "score": format_reward + answer_score + tool_reward + RaRTK_reward,
            "reward_parts": {
