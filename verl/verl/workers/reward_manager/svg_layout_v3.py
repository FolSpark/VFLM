...
class SvgLayoutRewardManager:
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        for i in range(len(data)):
            ...
            indexed_data[extra_info["index"]].append(1 if len(tool_result_images) > 2 else 0)

            
            reward = tool_result_images_scores[-1]
            reward_tensor[i, valid_response_length - 1] = reward
            reward_extra_info["reward_parts"].append({
                "reward": reward,
                "first": tool_result_images_scores[0],
                "tool_call_mean": sum(tool_result_images_scores[:-1]) / (len(tool_result_images_scores) - 1) if len(tool_result_images_scores) > 1 else 0,
