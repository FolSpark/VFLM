...
def compute_advantage(
    ...
    with torch.no_grad():
        ...
        # 0. 不归一化，extra_advantage_tensor 直接用
        ...
        # 1. 全局归一化, 先把 norm_adv_by_std_in_grpo 设置为False
        scores = (scores.unsqueeze(-1) * response_mask + extra_advantage_tensor) * response_mask
        scores = verl_F.masked_whiten(scores, response_mask, shift_mean=True) * response_mask
        ...
        # scores = (scores.unsqueeze(-1) * first_turn_response_mask + middle_turn_advantage_tensor * middle_turn_response_mask + last_turn_advantage_tensor * last_turn_response_mask) * response_mask
        # 最后一轮这样还是不合理！！！！！
...
class SvgLayoutResultRewardManager:
    """The reward manager."""
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        for i in range(len(data)):
            ...
            # indexed_data[extra_info["index"]]["answer_scores"].append(tool_result_images_scores[-1]["total_score"])
        ...
        # for i in range(len(data)):
            ...
            reward = tool_result_images_scores[-1]["total_score"]   # -1是最后的，0为第一轮的
            ...
            # answer_punishment = (tool_result_images_scores[-1]["total_score"] - max(indexed_data[extra_info["index"]]["answer_scores"])) * max(0.0, 4.0 - len(tool_result_images_scores)) if len(tool_result_images_scores) > 1 else 0.0  # 最多是5，小于5是不到最大都惩罚，小于4是最后两次有灵活
            ...
            reward_extra_info["reward_parts"].append({
                "answer-total_score": tool_result_images_scores[-1]["total_score"],
                "answer-image_score": tool_result_images_scores[-1]["image_score"],
                "answer-ocr_accuracy": tool_result_images_scores[-1]["ocr_accuracy"],
                "answer-svg_code_accuracy": tool_result_images_scores[-1]["svg_code_accuracy"],
                # "RaRTK_reward": RaRTK_reward,
                "first-total_score": tool_result_images_scores[0]["total_score"],
                "first-image_score": tool_result_images_scores[0]["image_score"],
                "first-ocr_accuracy": tool_result_images_scores[0]["ocr_accuracy"],
                "first-svg_code_accuracy": tool_result_images_scores[0]["svg_code_accuracy"],
                "tool_score_mean": sum([x["total_score"] for x in tool_result_images_scores[:-1]]) / (len(tool_result_images_scores) - 1) if len(tool_result_images_scores) > 1 else 0,
                "second-first": tool_result_images_scores[1]["total_score"] - tool_result_images_scores[0]["total_score"] if len(tool_result_images_scores) > 1 else 0,
                "final-first": tool_result_images_scores[-1]["total_score"] - tool_result_images_scores[0]["total_score"],
                "valid-second-first": tool_result_images_scores[1]["total_score"] - tool_result_images_scores[0]["total_score"] if len(tool_result_images_scores) > 2 else -100,
                "valid-final-first": tool_result_images_scores[-1]["total_score"] - tool_result_images_scores[0]["total_score"] if len(tool_result_images_scores) > 2 else -100,
                "second-first-image_score": tool_result_images_scores[1]["image_score"] - tool_result_images_scores[0]["image_score"] if len(tool_result_images_scores) > 1 else 0,
                "final-first-image_score": tool_result_images_scores[-1]["image_score"] - tool_result_images_scores[0]["image_score"],
                "valid-second-first-image_score": tool_result_images_scores[1]["image_score"] - tool_result_images_scores[0]["image_score"] if len(tool_result_images_scores) > 2 else -100,
                "valid-final-first-image_score": tool_result_images_scores[-1]["image_score"] - tool_result_images_scores[0]["image_score"] if len(tool_result_images_scores) > 2 else -100,
