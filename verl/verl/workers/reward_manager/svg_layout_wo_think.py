...
class NaiveSvgLayoutRewardManager:
    """The reward manager."""
    ...
    def compute_score(self, param) -> Dict[str, Any]:
        ...
        if answer_str:
            if answer_str.startswith("```svg\n") or answer_str.startswith("```xml\n"):
                answer_str = answer_str[7:]
            elif answer_str.startswith("```html\n"):
            ...
            print(f"No SVG code found in the response: {response_str}")
            rendered_img = Image.new("RGB", bg_image.size, (0, 0, 0))
        
        answer_score = self.compute_image_score(
            svg_code=answer_str if answer_str else "",
        ...
        return {
            "index": param["index"],
            # "score": image_score,
            "score": image_score + 0.25 * ocr_accuracy + 0.25 * svg_code_accuracy,
            "reward_parts": {
